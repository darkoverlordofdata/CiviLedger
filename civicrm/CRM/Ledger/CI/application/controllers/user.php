...
class User extends CI_Controller {
    ...
    function login()
    {
        if ($this->config->item('userlogin') == "CMS") {
            /*
             * Accept the user logged into the hosting CMS (Joomla!)
             */
            ...
            {
                $this->messages->add('User '.$data_user_name.' not authorized', 'error');
                $this->template->load('user_template', 'user/logout', $data);
                return;
            }
            ...
            {
                $this->messages->add('User '.$data_user_name.' disabled.', 'error');
                $this->template->load('user_template', 'user/logout', $data);
                return;
            }
    
            $this->messages->add('Active account ' . $active_user['account'] . '.', 'success');
            ...
            $this->session->set_userdata('active_account', $active_user['account']);
