...
class General {
	...
	function check_account($account_name)
	{
        ...
        if ( ! isset($account_data['use_cms']))
        {
            $CI->messages->add('CMS flag missing from account setting file. Default MySQL port is 3306.', 'error');
            return FALSE;
        }
		if ( ! isset($account_data['db_prefix']))
		{
			$CI->messages->add('Database prefix missing from account setting file. Default MySQL port is 3306.', 'error');
			return FALSE;
		}
		...
		if ($CI->db->conn_id)
		{
			...
			if ($CI->db->query("SHOW TABLES"))
			{
				...
				if ($CI->uri->segment(1) != "update")
				{
					...
					$valid_settings_q = mysql_query('DESC webzash_settings');
					if ( ! $valid_settings_q)
					{
					...
					$table_names = array('webzash_groups', 'webzash_ledgers', 'webzash_entry_types', 'webzash_entries', 'webzash_entry_items', 'webzash_tags', 'webzash_logs', 'webzash_settings');
	...
	function check_setting()
	{
        ...
        $CI->config->set_item('userlogin', "CMS");
		...
		if (get_file_info($setting_ini_file))
		{
			...
			if ($cur_setting)
			{
                ...
                {
                    $CI->config->set_item('userlogin', $cur_setting['userlogin']);
                }
			}
		}
	}
	...
	function check_user($user_name)
	{
		...
		if ( ! get_file_info($ini_file))
		{
			$CI->messages->add('User does not exist.', 'error');
			return FALSE;
		}
        ...
        {
            $CI->messages->add('Current account missing from user file.', 'error');
        }
		return $user_data;
	}
