...
class Accountlist
{
    ...
    function add_sub_ledgers()
    {
        ...
        $CI->db->from('ledgers')->where('group_id', $this->id)->order_by('name', 'asc');
    ...
    function account_st_short($c = 0)
    {
        ...
        if ($this->id != 0)
        {
            ...
            echo "&nbsp;" .  $this->name;
            echo "</td>";
            echo "<td class=\"td-group\" align=\"right\">" . convert_amount_dc($this->total) . $this->print_space($this->counter) . "</td>";
            echo "</tr>";
        }
